...
\Fn{\texttt{verificar\_y\_migrar\_cadena}(rBlock, status)}{
	...
	\texttt{MPI\_Send}($rBlock.block\_hash$, $status.mpi\_source$, \texttt{TAG\_CHAIN\_HASH}) \\
...
\textbf{TODO Sacar \texttt{valid\_new\_block($b$)}? Igual habría que revisar que los hashes sean correctos en algún lugar? No dice nada en el protocolo creo. Mensaje con hash en vez de bloque.}
